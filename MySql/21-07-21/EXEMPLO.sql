...
select * from tb_produtos where dtvalidade between '2021-11-15' and '2021-11-15' order by dtvalidade, nome;
...
select count(categoria_id) from tb_produtos;
 ...
 select * from tb_produtos where preco = (select max(preco) as Preco_Maximo from tb_produtos);
